        ...
        for i, user_id in enumerate(tqdm(group.index)):
            ...
            if len(content_id) > self.max_seq:
                ...
                if initial >= ACCEPTED_USER_CONTENT_SIZE:
                    ...
                    self.samples[f"{user_id}_0"] = (content_id[:initial], answered_correctly[:initial])
                
                for seq in range(total_questions // self.max_seq):
                    self.user_ids.append(f"{user_id}_{seq+1}")
                    start = initial + seq * self.max_seq
                    end = start + self.max_seq
                    self.samples[f"{user_id}_{seq+1}"] = (content_id[start:end], answered_correctly[start:end])
            else:
                user_id = str(user_id)
                self.user_ids.append(user_id)
                self.samples[user_id] = (content_id, answered_correctly)
