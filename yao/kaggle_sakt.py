...
def compute_cooccurrence(data):
    ...
    for tid, tags in enumerate(data):
        ...
        for tag_1, tag_2 in list(combinations_with_replacement(tags_list, 2)):
            # tag is 1-based
            if tag_1 == tag_2:
                cooccurrence_matrix[tag_1 - 1][tag_2 - 1] += 1
            else:
                cooccurrence_matrix[tag_1 - 1][tag_2 - 1] += 1
                cooccurrence_matrix[tag_2 - 1][tag_1 - 1] += 1
    ...
    tags_embedding = np.concatenate([padding_vector, svd.components_.T], axis=0)

    return tags_embedding, all_tags
...
# tags_embedding, parsed_tags = compute_cooccurrence(question_df['tags'].values)
# question_df['tags'] = parsed_tags
# print(question_df.head())
# print(tags_embedding[0:3], tags_embedding.shape)
# 1/0
...
model = MD.SAKTModel(n_skill, max_seq=MAX_SEQ, embed_dim=EMBED_DIM, dropout=DROPOUT_RATE, enc_layers=1)
